...
const fetchadminnew = require("../middleware/fetchadminnew");
  ...
  router.delete("/newcomplaina",[
   ...
   try{
    ...
    res.json({message:"Deleted ",response:true})
        ...
        console.log(error)
        res.status(500).json({message:req.body.id+'please try again latter',response:false})
      }
   
  
  });

  router.delete("/delfeedback",[
    body("id", "id is required").exists(),
  ],async (req,res)=>{
    const errors = validationResult(req);
    if(!errors.isEmpty()){
   ...
   try{
    //let ugatet = await Feedback.deleteOne({_id:req.body.id});
    let ugatet = await Feedback.updateOne({_id:req.body.id},{$set: {read: true}});
    res.json({message:"Deleted ",response:true})
    }
