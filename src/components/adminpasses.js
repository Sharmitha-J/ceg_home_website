...
const sendrq=async(props)=>{
  ...
  if(whatid==0){
    const response=await fetch(`https://ceg-home-website.onrender.com/api/ud/updateintime`,{
      method:'POST',
      headers:{
          'Content-Type':'application/json',
  ...
  const response=await fetch(`https://ceg-home-website.onrender.com/api/ud/updateouttime`,{
          method:'POST',
          headers:{
              'Content-Type':'application/json',
              'auth-token':localStorage.getItem('admintoken')
